...
import { DomainRow } from '@/types/reports'
import { formatNumber, formatCTR } from '@/lib/dateUtils'
import VisibilityToggle from './VisibilityToggle'

function DomainFavicon({ domain, rank }: { domain: string; rank: number }) {
...
export default function DomainBreakdownCards({
  data,
  totalImpressions,
  isImpersonating,
  isRowHidden,
  ...
  const visibleTop10 = isImpersonating
    ? top10
    : top10.filter(row => !(isRowHidden?.(row.domain) ?? false))
  ...
  if (sorted.length === 0) {
    return (
      <div className="px-5 pb-5 text-center text-[#64748b] text-sm py-8">No domain data available.</div>
    )
  }
    ...
    <div className="px-5 pb-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {visibleTop10.map((row, i) => {
          const hidden = isRowHidden?.(row.domain) ?? false
          ...
          return (
            ...
            >
              <DomainFavicon domain={row.domain} rank={i + 1} />
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    {isImpersonating && (
                      <VisibilityToggle
                        isHidden={hidden}
                        onToggle={() => onToggleRow?.(row.domain, !hidden)}
                      />
                    )}
                  </div>
                </div>
      ...
      {sorted.length > 10 && (
        <div className="mt-4 text-center">
          ...
          >
            View All {sorted.length} Domains
          </button>
        </div>
      )}
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="text-base font-bold text-gray-900">All Domains ({sorted.length})</h3>
              <button
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-white border-b border-gray-100">
                  <tr>
                    ...
                    <th className="text-left px-6 py-2.5 text-xs font-semibold text-[#64748b] uppercase tracking-wide">Domain</th>
                ...
                <tbody>
                  {filtered.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-2.5 font-medium text-gray-900 max-w-xs truncate" title={row.domain}>{row.domain}</td>
