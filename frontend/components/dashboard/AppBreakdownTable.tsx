...
export default function AppBreakdownTable({ data, isImpersonating, isRowHidden, onToggleRow }: Props) {
  ...
  if (sorted.length === 0) {
    return (
      <div className="px-5 py-8 text-center text-[#64748b] text-sm">No app data available.</div>
    )
  }
    ...
    <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ backgroundColor: '#f8fafc' }}>
              <th className="text-left px-5 py-3 text-xs font-semibold text-[#64748b] uppercase tracking-wide">App</th>
          ...
          <tbody>
            {displayed.map((row, i) => {
              ...
              return (
                ...
                >
                  <td className="px-5 py-3 font-medium text-gray-900 max-w-xs truncate" title={row.app}>{row.app || '—'}</td>
      ...
      {sorted.length > 10 && (
        <div className="px-5 py-3 border-t border-gray-100">
          ...
          >
            {showAll ? 'Show Top 10' : `Show All ${sorted.length} Apps`}
          </button>
        </div>
      )}
    </div>
