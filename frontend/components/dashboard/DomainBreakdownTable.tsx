...
export default function DomainBreakdownTable({ data, isImpersonating, isRowHidden, onToggleRow }: Props) {
    ...
    <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ backgroundColor: '#f8fafc' }}>
              <th className="text-left px-5 py-3 text-xs font-semibold text-[#64748b] uppercase tracking-wide">Domain</th>
          ...
          <tbody>
            {displayed.map((row, i) => {
              ...
              return (
                ...
                >
                  <td className="px-5 py-3 font-medium text-gray-900 max-w-xs truncate" title={row.domain}>{row.domain || '—'}</td>
            ...
            {sorted.length === 0 && (
              <tr>
                <td colSpan={isImpersonating ? 5 : 4} className="px-5 py-8 text-center text-[#64748b] text-sm">No domain data available.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {sorted.length > 10 && (
        <div className="px-5 py-3 border-t border-gray-100">
          ...
          >
            {showAll ? 'Show Top 10' : `Show All ${sorted.length} Domains`}
          </button>
        </div>
      )}
    </div>
